 ...
 *      1） 需求①②容易改写。
 *      2）需求③
 *      方案一：可以新增数据项all，表示有多少数经过了当前节点。改写需要注意同时调整size和all。add方法好改。delete方法不好改。（慎用）
 *      方案二：可以包装一个Node类型用来保存key和index（相同key也要有不同的index序号），比较时，key相同情况下用index比较。
 *          这种方案有序表不用考虑重复数据的问题。好改写。但某些情况下add操作不在一个循环内进行，无法做到无重复添加递增index。
